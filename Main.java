...
public class Main {
    ...
    private static String SplitUser;
    ...
    public static void main(String... sargs) {
       ...
       Aj clientArgs = new Aj(accountName, scriptName, scriptCommand, world, breakProfile,  1472396136); //Magic number best number
        DL var6 = new DL(new ArrayList<>(), null);
        var6.A.add(clientArgs);
        ...
        if (proxyIP != null && !proxyIP.equals(""))
            args.add("PROXY_HOST" + AA.yL(proxyIP.getBytes()));
        if (proxyPort != null && !proxyPort.equals(""))
            args.add("PROXY_PORT" + AA.yL(proxyPort.getBytes()));
        if (proxyUsername != null && !proxyUsername.equals(""))
            args.add("PROXY_USERNAME" + AA.yL(proxyUsername.getBytes()));
        if (proxyPassword != null && !proxyPassword.equals(""))
            args.add("PROXY_PASSWORD" + AA.yL(proxyPassword.getBytes()));

        args.add("BOOT_INFO" + AA.yL(bootname.getBytes()));
        args.add("OLDSCHOOL");
    ...
    public static String requestUrl(String urlString) {
        try {
            ...
            return("IOExcept Req: " + e.toString());
        }
